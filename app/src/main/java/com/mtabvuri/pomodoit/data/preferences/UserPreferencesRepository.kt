...
class UserPreferencesRepository(private val context: Context) {
    ...
    suspend fun updatePomodoroTime(pomodoroTime: Int) {
        context.dataStore.edit { preferences ->
            if (pomodoroTime in (POMODORO_TIME_LOWEST) until POMODORO_TIME_HIGHEST + 1)
    ...
    suspend fun updateShortBreakTime(breakTime: Int) {
        context.dataStore.edit { preferences ->
            if (breakTime in (SHORT_BREAK_TIME_LOWEST) until SHORT_BREAK_TIME_HIGHEST + 1)
    ...
    suspend fun updateLongBreakTime(breakTime: Int) {
        context.dataStore.edit { preferences ->
            if (breakTime in (LONG_BREAK_TIME_LOWEST) until LONG_BREAK_TIME_HIGHEST + 1)
    ...
    private fun mapUserPreferences(preferences: Preferences): UserPreferences {
        ...
        val pomodoroTime = preferences[POMODORO_TIME_KEY] ?: POMODORO_TIME_DEFAULT
        val shortBreakTime = preferences[SHORT_BREAK_TIME_KEY] ?: SHORT_BREAK_TIME_DEFAULT
        val longBreakTime = preferences[LONG_BREAK_TIME_KEY] ?: LONG_BREAK_TIME_DEFAULT
